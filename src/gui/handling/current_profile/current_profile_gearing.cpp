...
namespace big
{
	void tab_current_profile::tab_gearing()
	{
		if (ImGui::BeginTabItem("Gearing"))
		{
			...
			ImGui::DragFloat("###handling_upshift", &g_local_player->m_vehicle->m_handling_data->m_upshift, .01f, 0.f, 50.f);

			ImGui::Text("Downshift Multiplier");
			ImGui::DragFloat("###handling_downshift", &g_local_player->m_vehicle->m_handling_data->m_downshift, .01f, 0.f, 50.f);
